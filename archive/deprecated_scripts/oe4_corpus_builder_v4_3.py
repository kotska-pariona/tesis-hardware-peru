"""
OE4 — corpus_builder_v4.3  (FINAL)
Fixes sobre v4.2:
  1. Protección bidireccional: EN_RIESGO léxico fuerte → no puede caer a OBSOLETO
     solo por condición "used"
  2. Texto enriquecido: omitir trend si abs > 40 (ya estaba en scoring, falta en texto)
  3. Columna price_trend_pct: marcar como NaN si abs > 40 (dato no confiable)
"""
import re, json
import numpy as np
import pandas as pd
...
MIN_TITLE   = 25
N_PER_CLASS = 1000
TREND_MAX   = 40.0   # trends > ±40% = ruido multi-fuente → ignorar
...
GEN_RULES = {
    ...
    "vigente": [
        r'\bddr5\b', r'\blpddr5\b',
        r'\brtx\s*50[0-9]{2}\b', r'\brtx\s*[56][0-9]{3}\b',
        r'\brx\s*9[0-9]{3}\b',
        r'\bcore\s*ultra\b', r'\bintel\s*arc\b',
        r'\bryzen\s*[579]\s*9[0-9]{3}\w*\b',
        r'\bryzen\s*[579]\s*8[0-9]{3}\w*\b',
        r'\blga\s*1851\b', r'\bam5\b',
        r'\bpcie\s*5\.0\b', r'\bpcie\s*5\b',
        r'\bwindows\s*11\b',
        r'\bi[357]-[12][0-9]{4}\w*\b',
        r'\bwifi\s*7\b', r'\bwi-fi\s*7\b',
        r'\bgen\s*5\b',
    ],
}
...
def label_sku(row: pd.Series) -> tuple:
    ...
    trend_clean = abs(price_trend) < TREND_MAX

    gen = gen_score(title)

    # ── Protección bidireccional ─────────────────────────────────────────
    # VIGENTE léxico fuerte (>=2) + used → máximo EN_RIESGO
    # EN_RIESGO léxico fuerte (>=2) + used → se queda EN_RIESGO, no cae a OBSOLETO
    lex_vigente_strong   = gen["vigente"]   >= 2
    lex_en_riesgo_strong = gen["en_riesgo"] >= 1
    lex_protected        = lex_vigente_strong or lex_en_riesgo_strong

    market = {"vigente": 0, "en_riesgo": 0, "obsoleto": 0}

    if cond_used > 0.5:
        if lex_protected:
            market["en_riesgo"] += 2   # ← protección: usado pero gen reciente
        else:
            market["obsoleto"] += 3
    elif cond_used > 0.2:
        market["en_riesgo"] += 2
    ...
    razon = best
    if lex_protected and cond_used > 0.5:
        razon = f"{best}_usado_protegido"

    return label_map[best], round(conf, 3), razon
...
def build_texto(row: pd.Series) -> str:
    ...
    # Solo incluir señales numéricas si son confiables
    if disc > 0 and abs(trend) < TREND_MAX:
        parts.append(f"descuento promedio: {disc:.1f}%")
    if abs(trend) > 1 and abs(trend) < TREND_MAX:
...
def main():
    t0 = datetime.now()
    print("=" * 60)
    print("  OE4 corpus_builder_v4.3 — FINAL")
    ...
    print(f"  Filas válidas: {len(df):,} | SKUs: {df['sku'].nunique():,}")
    ...
    def agg_sku(g):
        ...
        if len(prices) >= 2 and float(prices.iloc[0]) > 0:
            ...
            trend = 0.0
        # Marcar trend no confiable como NaN
        trend_clean = trend if abs(trend) < TREND_MAX else float("nan")
        cond     = g["condition"].fillna("New")
        pct_used = float((cond.isin(["Used","Open box"])).mean())
        t_cands  = g["title"][g["title"].str.len() >= MIN_TITLE]
        t_repr   = t_cands.mode().iloc[0] if len(t_cands) > 0 else g["title"].mode().iloc[0]
        return pd.Series({
            "n_obs"          : len(g),
            "title_repr"     : t_repr,
            ...
            "price_trend_pct": round(trend_clean, 2) if not np.isnan(trend_clean) else None,
    ...
    n_trend_null = df_skus["price_trend_pct"].isna().sum()
    print(f"  SKUs válidos: {len(df_skus):,}")
    print(f"  Trends marcados como NaN (ruido): {n_trend_null:,} ({n_trend_null/len(df_skus)*100:.1f}%)")
    ...
    print(f"  Etiquetados: {len(df_labeled):,} / {len(df_skus):,}")
    print(f"  Distribución bruta:")
    for lid, lname in LABEL_NAMES.items():
        n   = (df_labeled["label_id"] == lid).sum()
        pct = n / len(df_labeled) * 100
        print(f"    {lname:12s}: {n:>6,}  ({pct:.1f}%)")

    # Auditoría protección
    prot = df_labeled[df_labeled["razon"].str.contains("protegido", na=False)]
    print(f"\n  🔧 Casos con protección léxica: {len(prot)}")
    for _, r in prot.head(3).iterrows():
        print(f"    [{r['label']}] {str(r['title_repr'])[:65]}")
        print(f"           razón={r['razon']} | used={r['pct_used']}")

    # Validación: DDR4 used → debe ser EN_RIESGO
    print(f"\n  🔍 Validación DDR4 + used:")
    ddr4_used = df_labeled[
        df_labeled["title_repr"].str.lower().str.contains(r'\bddr4\b', regex=True, na=False) &
        (df_labeled["pct_used"] > 0.5)
    ]
    vc = ddr4_used["label"].value_counts()
    print(f"    {vc.to_dict()}")
    ...
    report = {
        "version"          : "v4.3-FINAL",
        "unidad_analisis"  : "SKU (modelo) con protección léxica bidireccional",
        "timestamp"        : datetime.now().isoformat(),
        "corpus_rows"      : len(df_bal),
        "actual_counts"    : actual_counts,
        "coverage_pct"     : round(len(df_labeled)/len(df_skus)*100, 2),
        "casos_protegidos" : len(prot),
    ...
    print("\n📊 Confianza por etiqueta (v4.3):")
    print(df_bal.groupby("label")["confianza"].describe().round(3).to_string())

    print(f"\n✅ Corpus v4.3 FINAL listo en {elapsed:.1f}s")
    print("🔜 Siguiente: oe4_e5_embedder_v2.py → corpus_obsolescencia_v4.csv")
