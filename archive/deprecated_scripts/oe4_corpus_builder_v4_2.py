"""
OE4 — corpus_builder_v4.2
Fix: conflicto léxico-mercado resuelto con prioridad léxica cuando conf >= 0.67
     + HP EliteBook Core Ultra "used" → EN_RIESGO (no OBSOLETO)
     + Regla: si gen_score vigente >= 2 → no puede ser OBSOLETO por sola condición
"""
import re, json
import numpy as np
import pandas as pd
...
GEN_RULES = {
    ...
    "vigente": [
        r'\bddr5\b', r'\blpddr5\b',              # ← NUEVO: LPDDR5
        r'\brtx\s*50[0-9]{2}\b', r'\brtx\s*[56][0-9]{3}\b',
        r'\brx\s*9[0-9]{3}\b',
        r'\bcore\s*ultra\b', r'\bintel\s*arc\b', # ← NUEVO: Intel Arc
...
def label_sku(row: pd.Series) -> tuple:
    ...
    # ── Regla de protección: si léxico dice VIGENTE con fuerza ──────────
    # Un producto con señales vigentes claras NO puede ser OBSOLETO
    # solo por condición "used" — máximo EN_RIESGO
    lex_vigente_strong  = gen["vigente"]  >= 2
    lex_obsoleto_strong = gen["obsoleto"] >= 2

    market = {"vigente": 0, "en_riesgo": 0, "obsoleto": 0}

    # Condición — con protección léxica
    if cond_used > 0.5:
        if lex_vigente_strong:
            market["en_riesgo"] += 2   # vigente usado → EN_RIESGO, no OBSOLETO
        else:
            market["obsoleto"] += 3
    elif cond_used > 0.2:
        market["en_riesgo"] += 2

    # Señales de precio (solo si trend limpio)
    ...
    # Razón detallada para auditoría
    razon = best
    if lex_vigente_strong and cond_used > 0.5:
        razon = "vigente_usado→en_riesgo"
...
def main():
    t0 = datetime.now()
    print("=" * 60)
    print("  OE4 corpus_builder_v4.2 — Protección léxica anti-conflicto")
    ...
    print("\n🏷️  Etiquetando con protección léxica...")
    ...
    # Auditoría de casos corregidos
    corr = df_labeled[df_labeled["razon"] == "vigente_usado→en_riesgo"]
    print(f"\n  🔧 Casos corregidos (vigente_usado→en_riesgo): {len(corr)}")
    if len(corr) > 0:
        for _, r in corr.head(3).iterrows():
            print(f"    {str(r['title_repr'])[:70]}")
    ...
    report = {
        "version"        : "v4.2",
        "unidad_analisis": "SKU (modelo) con protección léxica",
    ...
    print(f"\n✅ Corpus v4.2 listo en {elapsed:.1f}s")
    print("🔜 Siguiente: oe4_e5_embedder_v2.py")
