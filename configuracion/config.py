...
Fixes v2.0:
  ...
  - [FIX-4] config.py movido a agent/ — mismo directorio que roi_calculator.py
...
# ── Directorios base ──────────────────────────────────────────────────────
# agent/config.py → ROOT = agent/../ = raíz del repo
...
DATA_RAW_DIR       = Path(os.getenv("DATA_RAW_DIR",  str(DATA_DIR / "raw")))       # FIX-1
DATA_PROCESSED_DIR = Path(os.getenv("DATA_PROC_DIR", str(DATA_DIR / "processed"))) # FIX-7
MODELS_DIR         = Path(os.getenv("MODELS_DIR",    str(DATA_DIR / "models")))
LOGS_DIR           = Path(os.getenv("LOGS_DIR",      str(DATA_DIR / "logs")))      # FIX-3

for _d in [DATA_DIR, DATA_RAW_DIR, DATA_PROCESSED_DIR, MODELS_DIR, LOGS_DIR]:
    _d.mkdir(parents=True, exist_ok=True)

# ── Archivos de datos ─────────────────────────────────────────────────────
# FIX-2: MASTER_CSV apunta a data/raw/ — donde main.py lo escribe
MASTER_CSV          = DATA_RAW_DIR / "MASTER_hardware_peru.csv"

# Legacy — mantenidos para compatibilidad con código existente
